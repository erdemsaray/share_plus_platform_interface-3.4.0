...
class MethodChannelShare extends SharePlatform {
  ...
  Future<ShareResult> share(
    ...
    var result = await channel.invokeMethod<ShareResult>('share', params);
    return result ?? ShareResult("", ShareResultStatus.dismissed);
  }
  ...
  Future<ShareResult> shareFilesWithResult(
    ...
    final result =
        await channel.invokeMethod<String>('shareFilesWithResult', params) ?? 'dev.fluttercommunity.plus/share/unavailable';
  ...
  Future<XFile> _getFile(XFile file, {String? tempRoot}) async {
    if (file.path.isNotEmpty) {
      ...
      final filename = file.name.isNotEmpty // If filename exists
              ||
              lookupMimeType(file.name) != null //If the filename has a valid extension
          ? file.name
  ...
  Future<List<XFile>> _getFiles(List<XFile> files) async => await Future.wait(files.map((entry) => _getFile(entry)));
